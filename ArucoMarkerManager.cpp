...
ArucoMarkerManager::ArucoMarkerManager(const cv::Mat& camMatrix, const cv::Mat& distCoeff, const int& dictionaryId) : cameraMatrix(camMatrix), distCoeffs(distCoeff) {
	dictionary = cv::makePtr<cv::aruco::Dictionary>(cv::aruco::getPredefinedDictionary(dictionaryId));
}

void ArucoMarkerManager::detectAndEstimatePose(const cv::Mat& frame, std::vector<int>& ids, std::vector<std::vector<cv::Point2f>>& corners, const double& length, std::vector<cv::Vec3d>& rvecs, std::vector<cv::Vec3d>& tvecs) {
...
void ArucoMarkerManager::drawDetectedMarkers(cv::Mat& frame, const std::vector<std::vector<cv::Point2f>>& corners, const std::vector<int>& ids) {
    cv::aruco::drawDetectedMarkers(frame, corners, ids);
}

void ArucoMarkerManager::drawAxis(const cv::Mat& image, const cv::Vec3d& rvec, const cv::Vec3d& tvec, const float& length) {
...
void ArucoMarkerManager::drawCube(const cv::Mat& image, const cv::Vec3d& rvec, const cv::Vec3d& tvec, const float& sideLength) {
...
cv::Ptr<cv::aruco::Dictionary> ArucoMarkerManager::getDictionary() const {
	return dictionary;
}